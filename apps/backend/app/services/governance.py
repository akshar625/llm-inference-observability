    ...
    async def _check_rate_limit(conversation_id: UUID) -> None:
        ...
        try:
            ...
            if count > settings.RATE_LIMIT_RPM:
                raise HTTPException(
                    status_code=429,
                    detail=f"Rate limit exceeded: max {settings.RATE_LIMIT_RPM} requests/minute per conversation.",
                )
        ...
        if estimated_tokens > settings.MAX_INPUT_TOKENS:
            raise HTTPException(
                ...
                detail=(
                    ...
                    f"the {settings.MAX_INPUT_TOKENS}-token limit. Shorten your message or start a new conversation."
                ),
            )
